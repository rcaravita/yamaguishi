	...
	def order
		orders.map do |admin_order|
			[
				...
				admin_order.delivery ? (admin_order.client ? admin_order.client.route.try(:name) : "") : (admin_order.pickup == 1 ? "RETIRADA Sítio" : (admin_order.pickup == 2 ? "RETIRADA Feira Bosque" : (admin_order.pickup == 3 ? "RETIRADA Pq. Ecológico" : (admin_order.pickup == 4 ? "RETIRADA Feira de Holambra" : "RETIRADA Centro de Convivência")))),
				...
				[2,3].include?(admin_order.status) ? check_box_tag(:atendido, true, admin_order.status == 3, disabled: true, name: admin_order.id) : "",
